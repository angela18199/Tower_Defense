...
void PlayScene::Update(float deltaTime) {
    ...
    switch (current.first) {
        ...
        case 4:
            EnemyGroup->AddNewObject(enemy = new Tank3GunEnemy(SpawnCoordinate.x, SpawnCoordinate.y));
            break;
        default:
            return;
    }
...
void PlayScene::ReadMap() {
    ...
    for (int i = 0; i < MapHeight; i++) {
        for (int j = 0; j < MapWidth; j++) {
            ...
            mapState[i][j] = num ? TILE_FLOOR : TILE_DIRT;   //mapData is 0 or 1
            if (num)  //if 1, block->floor-> walls
...
std::vector<std::vector<int>> PlayScene::CalculateBFSDistance() {
    ...
    std::vector<std::vector<int>> map(MapHeight, std::vector<int>(std::vector<int>(MapWidth, -1))); // 2D array map(MapHeigh, MapWidth) with all element -1 !?
    std::vector<int>();
    std::queue<Engine::Point> que;
    // Push end point.
    // BFS from end point.
    if (mapState[MapHeight - 1][MapWidth - 1] != TILE_DIRT)  // != 0  , invalid
        return map;
    
    //if is dirt = valid?
    que.push(Engine::Point(MapWidth - 1, MapHeight - 1));
    map[MapHeight - 1][MapWidth - 1] = 0;    //堡壘
    while (!que.empty()) {  //que is empty or not
        ...
        //check if 4 direction available
